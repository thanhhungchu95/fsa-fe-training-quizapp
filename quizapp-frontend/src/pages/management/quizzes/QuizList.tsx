...
import { faPlus, faRefresh, faSearch, faSortAlphaAsc, faSortAlphaDesc, faSortAmountAsc, faSortAmountDesc } from "@fortawesome/free-solid-svg-icons";
...
const QuizList = () => {
    const [items, setItems] = useState<QuizViewModel[]>([]); 
    ...
    const columns: TableColumnModel[] = [
        { field: 'thumbnailUrl', label: 'Picture', type: 'image', sortabled: false, iconSort: [], enum: false },
        { field: 'title', label: 'Title', type: 'string', sortabled: true, iconSort: [faSortAlphaAsc, faSortAlphaDesc], enum: null },
        { field: 'description', label: 'Description', type: 'string', sortabled: true, iconSort: [faSortAlphaAsc, faSortAlphaDesc], enum: null },
        { field: 'duration', label: 'Duration', type: 'duration', sortabled: true, iconSort: [faSortAmountAsc, faSortAmountDesc], enum: null },
        { field: 'numberOfQuestions', label: 'Number of Questions', type: 'number', sortabled: false, iconSort: [], enum: null },
    ...
    const searchData = useCallback(async() => {
        try {
            ...
            if (keyword) {
                Object.assign(filter, { title: keyword });
            }

            const response: any = await QuizService.search(filter);
    ...
    return (
        <section className="w-5/6">
            ...
            <TablePagination items={items} defaultOrderBy={'title'} pageInfo={pageInfo} columns={columns} onEdit={onEdit} onDelete={onDelete} onSearch={onSearch} />

            {/* Details Component */}
            <div id="detail-form" ref={detailForm}>
                {isShowDetail && (<QuizDetails item={selectedItem} onCancel={() => onCancelDetail()} />)}
            </div>
        </section>
    );
}
