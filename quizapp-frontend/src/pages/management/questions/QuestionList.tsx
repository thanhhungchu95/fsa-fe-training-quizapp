...
const QuestionList = () => {
    const [items, setItems] = useState<QuestionViewModel[]>([]); 
    const [loading, setLoading] = useState<boolean>(true);
    const [pageInfo, setPageInfo] = useState<PageInfo>(new PageInfo());
    const [page, setPage] = useState<number>(1);
    const [size, setSize] = useState<number>(5);
    const [orderBy, setOrderBy] = useState<string>('content');
    ...
    const columns: TableColumnModel[] = [
        { field: 'content', label: 'Content', type: 'string', sortabled: true, iconSort: [faSortAlphaAsc, faSortAlphaDesc], enum: null },
        { field: 'questionType', label: 'Question Type', type: 'enum', sortabled: true, iconSort: [faSortAlphaAsc, faSortAlphaDesc], enum: QuestionTypeEnum },
        { field: 'numberOfAnswers', label: 'Number of Answers', type: 'number', sortabled: false, iconSort: [], enum: null},
    ...
    const searchData = useCallback(async() => {
        try {
            ...
            if (keyword) {
                Object.assign(filter, { content: keyword });
            }

            if (questionType) {
                Object.assign(filter, { questionType: questionType });
            }

            const response: any = await QuestionService.search(filter);
    ...
    }, [page, size, orderBy, orderDirection, keyword, isActive, questionType]);
    ...
    const onDelete = async (item: any) => {
        let response: any;
        try {
            response = await QuestionService.remove(item.id);
        } catch (error) {
            BaseApiService.handleError(error);
        }
        if (response) {
    ...
    return (
        <section className="w-5/6">
            <div className="card border border-slate-300 rounded-md">
                ...
                <form onSubmit={handleSubmit}>
                    <div className="card-body p-3 border-y border-slate-300 flex flex-row">
                        ...
                        <div className="form-group w-1/3 flex flex-col ml-4">
                            <label htmlFor="questType" className="block mb-3">Question Type</label>
                            <select name="questionType" id="questionType" title="Select Question Type" onChange={(e) => setQuestionType(Number(e.target.value))}
            ...
            <TablePagination items={items} defaultOrderBy={'content'} pageInfo={pageInfo} columns={columns} onEdit={onEdit} onDelete={onDelete} onSearch={onSearch} />

            {/* Details Component */}
            <div id="detail-form" ref={detailForm}>
                {isShowDetail && (<QuestionDetails item={selectedItem} onCancel={() => onCancelDetail()} />)}
            </div>
        </section>
    );
}
