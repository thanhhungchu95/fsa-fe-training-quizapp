...
const UserList = () => {
    ...
    const columns: TableColumnModel[] = [
        ...
        { field: 'phoneNumber', label: 'Phone Number', type: 'string', sortabled: true, iconSort: [faSortAlphaAsc, faSortAlphaDesc], enum: null },
    ...
    return (
        <section className="w-5/6">
            ...
            <TablePagination items={items} defaultOrderBy={'email'} pageInfo={pageInfo} columns={columns} onEdit={onEdit} onDelete={onDelete} onSearch={onSearch} />

            {/* Details Component */}
            <div id="detail-form" ref={detailForm}>
                {isShowDetail && (<UserDetails item={selectedItem} onCancel={() => onCancelDetail()} />)}
            </div>
        </section>
    );
}
